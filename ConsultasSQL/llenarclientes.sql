...
UPDATE prueba.compensatorios SET 
            colaborador_id_compe = 1, 
            descripcion_compe = "Prueba DB Act", 
            inicio_compe = CONVERT(STR_TO_DATE("12/06/2024 05:00 PM", '%d/%m/%Y %l:%i %p'), DATETIME), 
            final_compe = CONVERT(STR_TO_DATE("12/06/2024 06:00 PM", '%d/%m/%Y %l:%i %p'), DATETIME), 
            validacion_compe = "Aceptado"  WHERE id_compe = 1;

            
#Prueba para crear compensatorio
